...
src_configure() {
	local mycmakeargs=(
		...
		-DCHIAKI_USE_SYSTEM_NANOPB=TRUE
		-DCHIAKI_USE_SYSTEM_JERASURE=TRUE
		-DCHIAKI_ENABLE_TESTS=$(usex test)
		-DCHIAKI_ENABLE_CLI=$(usex cli)
		-DCHIAKI_ENABLE_GUI=$(usex gui)
		-DCHIAKI_GUI_ENABLE_SDL_GAMECONTROLLER=$(usex sdl)  # Use SDL Gamecontroller for Input
		-DCHIAKI_ENABLE_SETSU=TRUE	#Enable libsetsu for touchpad input from controller
	)
